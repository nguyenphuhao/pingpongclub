...
1. [🆕 Unified Match Generation](#-unified-match-generation-new) - **RECOMMENDED** Single endpoint for all match generation
2. [Bracket Endpoints](#bracket-endpoints) - Single Elimination matches ⚠️ DEPRECATED
3. [Group Endpoints](#group-endpoints) - Round Robin matches ⚠️ DEPRECATED
4. [Generic Matches API](#generic-matches-api) - Unified matches API for both stages
5. [Participant Endpoints](#participant-endpoints)
6. [Tournament Management](#tournament-management)

---

# 🆕 Unified Match Generation (NEW)

> **⭐ RECOMMENDED**: Use this endpoint instead of separate bracket/group generation endpoints

## Generate Matches (Unified)

**Endpoint**: `POST /api/admin/tournaments/:id/matches/generate`

**Description**: Single endpoint to generate matches for both FINAL (bracket) and GROUP (round robin) stages.
...
- ✅ Round 2+: Virtual participants like "Thắng trận 1 (Vòng 1)"
...
- ✅ All participants are REAL (no virtual)
- ✅ Match status = `SCHEDULED`
- ✅ Group status → `IN_PROGRESS`

### API Comparison

| Old Endpoints (Deprecated) | New Unified Endpoint |
|---------------------------|----------------------|
| `POST /tournaments/:id/bracket/generate` | `POST /tournaments/:id/matches/generate` |
| `POST /tournaments/:id/groups/:gid/generate-matches` | `POST /tournaments/:id/matches/generate` |

### Migration Guide

**Before (Bracket)**:
```bash
POST /api/admin/tournaments/tour-123/bracket/generate
{ "includeThirdPlaceMatch": true }
```

**After (Unified)**:
```bash
POST /api/admin/tournaments/tour-123/matches/generate
{ "stage": "FINAL", "includeThirdPlaceMatch": true }
```

**Before (Group)**:
```bash
POST /api/admin/tournaments/tour-123/groups/group-A/generate-matches
{ "matchupsPerPair": 2 }
```
...
```bash
POST /api/admin/tournaments/tour-123/matches/generate
{ "stage": "GROUP", "groupId": "group-A", "matchupsPerPair": 2 }
```
...
> **⚠️ DEPRECATED**: These endpoints are deprecated. Use the [Unified Match Generation](#-unified-match-generation-new) endpoint instead.

> **For**: SINGLE_ELIMINATION tournaments và Final stage của TWO_STAGES tournaments

## 1. Generate Bracket (Official) ⚠️ DEPRECATED

**Endpoint**: `POST /api/admin/tournaments/:id/bracket/generate`

**⚠️ Deprecated**: Use `POST /tournaments/:id/matches/generate` with `stage: "FINAL"` instead.

**Description**: Tạo tất cả bracket matches chính thức (SCHEDULED) với placeholders cho rounds sau
...
**Behavior**:
- ✅ Tạo TẤT CẢ matches cho TẤT CẢ rounds ngay từ đầu
- ✅ Round 1: Gán real participants
- ✅ Round 2+: Không có participants (hiển thị TBD)
- ✅ Match status = `SCHEDULED`
- ✅ Tournament status → `IN_PROGRESS`

**Requirements**:
- Tournament status = `PENDING`
- Participants đã lock (`participantsLocked = true`)
- Chưa có bracket tồn tại

---

## 2. Get Bracket (với Placeholders)

**Endpoint**: `GET /api/admin/tournaments/:id/bracket`

**Description**: Lấy bracket structure với placeholders (TBD) cho visualization

**Response**: Giống như Generate Bracket response

**Behavior**:
- ✅ Trả về tất cả matches (kể cả TBD)
- ✅ Mỗi match có `nextMatchId` để link winners
- ✅ Participants không có → hiển thị "TBD"
- ✅ Compatible với @g-loot/react-tournament-brackets
...
**Description**: Tạo bracket ở chế độ DRAFT để preview/edit trước khi publish

**Request Body**:
```json
{
  "includeThirdPlaceMatch": true,
  "isDraft": true  // ← Set true để tạo draft
}
```

**Response**: Giống Generate Bracket nhưng match status = `DRAFT`

**Behavior**:
- ✅ Match status = `DRAFT`
- ✅ Tournament status KHÔNG thay đổi (giữ nguyên)
- ✅ Có thể edit participants, schedule
- ✅ KHÔNG yêu cầu tournament = PENDING
...
**Description**: Lấy draft bracket (chỉ DRAFT matches)
...
```json
{
  ...
  "data": {
    ...
    "totalRounds": 3,
    "totalMatches": 7,
    "matches": [/* Chỉ DRAFT matches */]
  }
}
```

**Behavior**:
- ✅ Chỉ trả về matches có status = `DRAFT`
- ✅ Trả về null nếu không có draft
...
**Description**: Edit một draft match (participants, schedule, round)
...
**Behavior**:
- ✅ Chỉ edit được matches có status = `DRAFT`
- ✅ Có thể đổi participants (swap matchups)
- ✅ Có thể đổi schedule
- ✅ Có thể đổi round number
...
**Description**: Publish draft bracket → chuyển tất cả DRAFT matches thành SCHEDULED
...
```json
{
  ...
  "data": {
    ...
    "bracket": {
      ...
      "totalRounds": 3,
      "totalMatches": 7,
      "matches": [/* Matches bây giờ là SCHEDULED */]
    }
  }
}
```
...
**Behavior**:
- ✅ Validate: Tất cả matches phải có valid participants (không có TBD)
- ✅ DRAFT → SCHEDULED (atomic)
- ✅ Tournament status → `IN_PROGRESS`
...
**Description**: Xóa tất cả draft matches
...
```json
{
  ...
  "data": {
    "deletedMatches": 7,
    "message": "Đã xóa 7 trận đấu nháp"
  }
}
```

**Behavior**:
- ✅ Chỉ xóa DRAFT matches
- ✅ Tournament status không thay đổi
- ✅ Có thể regenerate sau khi xóa
...
## 1. Generate Group Matches (Official) ⚠️ DEPRECATED

**Endpoint**: `POST /api/admin/tournaments/:id/groups/:gid/generate-matches`

**⚠️ Deprecated**: Use `POST /tournaments/:id/matches/generate` with `stage: "GROUP"` and `groupId` instead.

**Description**: Tạo round robin matches chính thức cho một group

**Request Body**:
```json
{
  "matchupsPerPair": 1,  // Mỗi cặp đấu mấy lần
  "isDraft": false       // Optional, default = false
}
```
...
**Behavior**:
- ✅ Tạo round robin matches (mọi người đấu với mọi người)
- ✅ Match status = `SCHEDULED`
- ✅ Group status → `IN_PROGRESS`
- ✅ Tất cả matches có đủ 2 participants (không có TBD)

**Requirements**:
- Group status = `PENDING`
- Group có ít nhất 2 participants
- Chưa có matches cho group
...
**Description**: Lấy thông tin group bao gồm participants và matches
...
**Description**: Tạo draft matches cho group để preview/edit

**Request Body**:
```json
{
  "matchupsPerPair": 1,
  "isDraft": true  // ← Set true
}
```

**Response**: Giống Generate nhưng match status = `DRAFT`

**Behavior**:
- ✅ Match status = `DRAFT`
- ✅ Group status KHÔNG thay đổi
- ✅ Có thể edit schedule
...
**Description**: Lấy draft matches của một group
...
**Description**: Edit một draft match trong group
...
**Response**: Match đã được update

**Behavior**:
- ✅ Chỉ edit DRAFT matches
- ✅ Participants phải thuộc group này
- ✅ KHÔNG thể đổi round (fixed trong round robin)
...
**Description**: Publish draft matches → SCHEDULED
...
**Behavior**:
- ✅ Validate: Tất cả matches có 2 participants
- ✅ DRAFT → SCHEDULED
- ✅ Group status → `IN_PROGRESS`
- ✅ Atomic transaction
...
**Description**: Xóa draft matches của group
...
```json
{
  ...
  "data": {
    "deletedMatches": 6,
    "message": "Đã xóa 6 trận đấu nháp cho bảng group-123"
  }
}
```
...
**Description**: Lấy bảng xếp hạng của group
...
**Key Features**:
- ✅ Unified endpoint for both FINAL and GROUP stage matches
- ✅ Empty `participants` array = TBD match (placeholder)
- ✅ `stage: "FINAL"` = Bracket match
- ✅ `stage: "GROUP"` = Group match
- ✅ `groupId: null` = Bracket match
- ✅ `groupId: "<id>"` = Group match
- ✅ `group: {...}` = Group info (only for GROUP stage)
...
- ✅ If `stage = "GROUP"`: `groupId` is required
- ✅ If `stage = "GROUP"`: participants must belong to specified group
- ✅ If participants provided: they must belong to tournament
- ✅ Participant positions must be unique (1 and 2)
...
|--------|----------|-------------|------------|
| GET | `/tournaments/:id/matches` | List all matches with filtering | ✅ |
| GET | `/tournaments/:id/matches/:matchId` | Get single match detail | ✅ |
| POST | `/tournaments/:id/matches` | Create match manually | ✅ |
| GET | `/tournaments/:id/matches/stats` | Get match statistics | ✅ |

**Key Benefits**:
- ✅ Unified API for both FINAL and GROUP stage matches
- ✅ Flexible filtering (stage, group, status, round)
- ✅ Pagination support
- ✅ TBD placeholder support
...
**Description**: Unlock để có thể thêm/xóa participants
...
**Request Body**: Giống Create nhưng tất cả fields optional
...
## 🆕 Unified Match Generation (1 endpoint) ⭐ RECOMMENDED

| Method | Endpoint | Description | Supported Stages |
|--------|----------|-------------|------------------|
| POST | `/tournaments/:id/matches/generate` | Generate matches for any stage | FINAL, GROUP |
...
**Key Features**:
- Single unified endpoint for all match generation
- Supports both FINAL (bracket) and GROUP (round robin) stages
- Cleaner, more consistent API design
- Virtual participant support for TWO_STAGES tournaments
...
## Bracket Endpoints (7 endpoints) ⚠️ DEPRECATED

> **⚠️ DEPRECATED**: Use [Unified Match Generation](#-unified-match-generation-1-endpoint--recommended) instead.

| Method | Endpoint | Description | Match Status | Tournament Status Change |
|--------|----------|-------------|--------------|-------------------------|
| POST | `/tournaments/:id/bracket/generate` ⚠️ | Generate official bracket (DEPRECATED) | SCHEDULED | → IN_PROGRESS |
...
| POST | `/tournaments/:id/bracket/draft/publish` | Publish draft | DRAFT → SCHEDULED | → IN_PROGRESS |
| DELETE | `/tournaments/:id/bracket/draft` | Delete draft | DRAFT | No change |

---

## Group Endpoints (8 endpoints) ⚠️ DEPRECATED

> **⚠️ DEPRECATED**: Use [Unified Match Generation](#-unified-match-generation-1-endpoint--recommended) instead.

| Method | Endpoint | Description | Match Status | Group Status Change |
|--------|----------|-------------|--------------|---------------------|
| POST | `/tournaments/:id/groups/:gid/generate-matches` ⚠️ | Generate official (DEPRECATED) | SCHEDULED | → IN_PROGRESS |
...
| POST | `/tournaments/:id/groups/:gid/draft/publish` | Publish draft | DRAFT → SCHEDULED | → IN_PROGRESS |
...
# ⚠️ Missing Endpoints (Future Implementation)
...
## Match Management
- ✅ ~~`GET /tournaments/:id/matches` - List all matches~~ **IMPLEMENTED**
- ✅ ~~`GET /tournaments/:id/matches/:matchId` - Get match detail~~ **IMPLEMENTED**
- ✅ ~~`POST /tournaments/:id/matches` - Create match manually~~ **IMPLEMENTED**
- ✅ ~~`GET /tournaments/:id/matches/stats` - Get match statistics~~ **IMPLEMENTED**
